...
**议题：Windows SDK 最低版本提升为 26100 并在启动时强校验**

在实际 dump 分析过程中发现，`26100` 之前的 Windows SDK / WinDbg / CDB 在加载某些 PDB 时会失败，表现为符号加载异常、分析链路不稳定，且问题并不容易由调用方从表面现象直接判断。
如果继续允许低版本 SDK 启动服务，会导致调用方把问题误判为 dump 损坏、符号服务器异常或命令本身失败。

**决策：**
- Windows SDK 最低要求提升为 `26100`
- 服务启动时主动解析实际使用的 `cdb.exe` 文件版本
- 若检测到 build 低于 `26100`，则直接报错退出，不允许带病启动

**原因：**
- `26100` 之前的 SDK 在某些 PDB 加载场景下存在已知兼容性问题
- 该问题发生在服务端调试器层，越早失败越容易定位
- 用启动期强校验替代运行期偶发失败，能降低排障成本
